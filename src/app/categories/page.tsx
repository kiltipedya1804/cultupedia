...
export const revalidate = 3600 // Régénère toutes les heures, pas de force-dynamic
      ...
      <main className="pt-24 pb-20">
        <div className="section">
            ...
            <section key={family.id} className="mb-14">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {getCategoriesByFamily(family.id).map(cat => (
                  <Link key={cat.id} href={`/categories/${cat.id}`} className="group card p-6 hover:-translate-y-1 transition-transform">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0">
                        <h3 className="font-display font-bold text-base mb-1.5 leading-tight group-hover:text-brand-rouge transition-colors" style={{ color: cat.color }}>
                          {cat.label.fr}
                        </h3>
                        <p className="text-xs text-[#9090A8] leading-relaxed line-clamp-2">{cat.description.fr}</p>
                      </div>
                    </div>
                  </Link>
                ))}
          ...
          <div className="text-center mt-12 pt-12 border-t border-black/[0.06]">
            ...
            <Link href="/search" className="btn-primary px-8 py-3.5 rounded-2xl">Recherche avancée →</Link>
          </div>
        </div>
      </main>
      <Footer lang="fr" />
